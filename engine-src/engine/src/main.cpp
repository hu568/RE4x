...
#endif
{
        ...
        || model.find(PATHSTR("models2")) != path_t::npos)
    {
        ...
        // SPANV2 (models/spanv2.*) receptive field is 33 (5 SPAB blocks with
        // 3x3 convs + tail 3x3 depthwise), so a 10 px prepadding would leave
        // visible seams at tile borders.  Bump it to 16 (= (33-1)/2).
        if (modelname.find(PATHSTR("spanv2")) != path_t::npos)
        {
            prepadding = 16;
        }
    }
